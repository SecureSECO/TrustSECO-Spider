...
class CVESpider(scrapy.Spider):
    name = 'CVE'
    start_urls = [str('https://cve.mitre.org/cgi-bin/cvekey.cgi?keyword=') + str(input('Enter the keyword to search on for security vulnerabilities:'))]
    ...
    def scrape_vulnerabilities(self, response):
        ...
        item['CVE_vulnerability_description'] = response.css('td').getall()[10].replace('<td colspan="2">', '').replace('</td>', '').strip()
